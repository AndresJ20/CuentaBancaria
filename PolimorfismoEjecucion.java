...
public class PolimorfismoEjecucion {
    ...
    public static void retirarAhorro(PolimorfismoCuenta [] cta){
        ...
        Scanner entrada = new Scanner(System.in); float valor_retiro, saldo;
        int numero_cuenta,indice = 0,posicion = -1; boolean encontrado = false;
        System.out.println("RETIRO DE AHORRO");
        System.out.println("*******");
        ...
        if(encontrado==true){
            ...
            System.out.println("\nINGRESAR VALOR DEL RETIRO: ");
            valor_retiro = entrada.nextFloat();
            if(valor_retiro>cta[posicion].getSaldo_actual()){
                System.out.println("NO SE PUEDE REALIZAR EL RETIRO. EL SALDO A RETIRAR ES MAYOR A SU SALDO ACTUAL");
            }else{
                saldo =cta[posicion].getSaldo_actual()-valor_retiro;
                if(saldo > cta[posicion].getAhorro_meta()){
                cta[posicion].setSaldo_actual(cta[posicion].getSaldo_actual()-valor_retiro);
                System.out.println("SALDO ACTUAL: "+cta[posicion].getSaldo_actual());
                System.out.println("\nDEPOSITO REALIZADO CON EXITO...");
                }else{
                    System.out.println("EL VALOR A RETIRAR ES MAYOR AL SALDO META");
                }
            }
        }
        else{
